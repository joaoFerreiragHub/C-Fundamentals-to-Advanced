...
let utilizadores = [
  ...
  },
  {
    id: 2,
    nome: "Dafne Palú",
    email: "dafnepalu@gmail.com",
    telefone: 923432123,
  },
  {
    id: 3,
    nome: "Thaissa Silva",
    email: "thaissasilva@gmail.com",
    telefone: 964534567,
  },
];
...
function mostraTabela(arrayUtilizadores) {
  ...
  let listaBotoesDelete = document.querySelectorAll(
    "#tabelaDados .fa-trash-can"
  );
  for (let botao of listaBotoesDelete) {
    botao.addEventListener("click", function () {
      if (confirm("Confirma a eliminação do utilizador")) {
        ...
        //   let arrayTemp = utilizadores.filter(
        //     (utilizador) => utilizador.id !== idAEliminar
        //   );
        //   utilizadores = [...arrayTemp];
        //   utilizadores=[...utilizadores.filter(utilizador => utilizador.id!==idAEliminar)];

        let posicaoNoArray = utilizadores.findIndex(
          (utilizador) => utilizador.id === idAEliminar
        );
...
document
  .querySelector("#formPesquisa")
  .addEventListener("submit", function (evento) {
    evento.preventDefault();
    let termoPesquisa = document.querySelector("#pesquisa").value;

    let arrayTemp = utilizadores.filter((utilizador) =>
      utilizador.nome
        .toLocaleUpperCase()
        .includes(termoPesquisa.toLocaleUpperCase())
    );
    mostraTabela(arrayTemp);
    document.querySelector("#pesquisa").value = "";
    // document.querySelector("#totalUtilizadores").textContent = arrayTemp.length;
  });
document
  .querySelector("#formPesquisa")
  .addEventListener("submit", function (evento) {
    ...
    let termoPesquisa = document.querySelector("#pesquisa").value;
    let arrayTemp = utilizadores.filter((utilizador) =>
      utilizador.nome.toUpperCase().includes(termoPesquisa.toUpperCase())
    );
    if (arrayTemp.length > 0) {
      mostraTabela(arrayTemp);
      document.querySelector("#msgPesquisa").style.display = "none";
...
document
  .querySelector("#btnLimpaPesquisa")
