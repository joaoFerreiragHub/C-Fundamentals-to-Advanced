...
let var1 = paises2[0].pais;
let var2 = paises2[0].linguas_oficiais.length;

for (let pos = 0; pos < paises2.length; pos++) {
  console.log(
    `${paises2[pos].pais} tem ${paises2[pos].linguas_oficiais.length} línguas oficiais.`
  );
}

for (let pais of paises2) {
  console.log(
    `${pais.pais} tem ${pais.linguas_oficiais.length} línguas oficiais.`
  );
}
