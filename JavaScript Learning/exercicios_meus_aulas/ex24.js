...
function mostraUser(userid) {
  ...
  fetch(`https://jsonplaceholder.typicode.com/users?id=${userid}`)
    ...
    .then((user) => {
      ...
      conteudo += `<p><strong>Userame:</strong> ${user[0].username}</p>`;
      conteudo += `<p><strong>E-mail:</strong> ${user[0].email}</p>`;
      conteudo += `<br><strong>Address:</strong><br>`;
...
function mostraTabela(lista) {
  ...
  for (let pos = 0; pos < lista.length; pos++) {
    let linha = `<tr>`;
    //"javascript:void[0]" → link nulo/void → não devolve nada
    linha += `<td><a href="javascript:void(0);" class="linkUserId" data-userid="${lista[pos].userId}"
      >${lista[pos].userId}</a></td>`;
  ...
  let listaLinks = document.querySelectorAll(".linkUserId");
